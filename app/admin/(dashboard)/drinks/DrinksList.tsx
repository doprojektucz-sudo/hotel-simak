...
import { deleteDrink, toggleDrinkActive, updateDrinkPrice } from "@/lib/actions/drinks";
...
export function DrinksList({ items, onEdit }: DrinksListProps) {
  const [deletingId, setDeletingId] = useState<string | null>(null);
  const [togglingId, setTogglingId] = useState<string | null>(null);

  const handleDelete = async (id: string, name: string) => {
    if (!confirm(`Opravdu chcete smazat "${name}"?`)) return;
  ...
  const handleToggleActive = async (id: string, currentState: boolean) => {
    setTogglingId(id);
    try {
      await toggleDrinkActive(id, !currentState);
    } finally {
      setTogglingId(null);
    }
  };
  ...
  const handlePriceUpdate = async (id: string, newPrice: number) => {
    await updateDrinkPrice(id, newPrice);
  };
          ...
          <div className="flex items-start justify-between gap-4">
            <div className="flex-1 min-w-0 cursor-pointer" onClick={() => onEdit(item)}>
              <div className="flex items-center gap-2 flex-wrap">
                {item.size && (
                  <span className="text-sm font-bold text-gray-500">{item.size}</span>
                )}
                <h4 className="font-medium text-gray-900">{item.name}</h4>
                {!item.isActive && (
                  <span className="text-xs bg-gray-100 text-gray-600 px-2 py-0.5 rounded">Skryté</span>
                )}
              </div>
              {item.description && (
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-1">
                <button
                  ...
                  title={item.isActive ? "Skrýt" : "Zobrazit"}
                >
                  {item.isActive ? (
